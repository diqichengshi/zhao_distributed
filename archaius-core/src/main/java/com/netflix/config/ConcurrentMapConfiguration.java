...
/**
 * This class uses a ConcurrentHashMap for reading/writing a property to achieve
 * high throughput and thread safety. The implementation is lock free for
 * {@link #getProperty(String)} and {@link #setProperty(String, Object)}, but
 * has some synchronization cost for {@link #addProperty(String, Object)} if the
 * object to add is not a String or the key already exists.
 * <p>
 * The methods from AbstractConfiguration related to listeners and event
 * generation are overridden so that adding/deleting listeners and firing events
 * are no longer synchronized. Also, it catches Throwable when it invokes the
 * listeners, making it more robust.
 * <p>
 * This configuration does not allow null as key or value and will throw
 * NullPointerException when trying to add or set properties with empty key or
 * value.
 *
 * @author awang
 *
...
public class ConcurrentMapConfiguration extends AbstractConfiguration {
	...
	protected void addPropertyDirect(String key, Object value) {
		...
		try {
			...
			if (previousValue instanceof List) {
				...
				// the previous value is replaced by a list containing the previous value and
				// the new value
	...
	/**
	 * Adds the specified value for the given property. This method supports single
	 * values and containers (e.g. collections or arrays) as well. In the latter
	 * case, {@link #addPropertyDirect(String, Object)} will be called for each
	 * element.
	 *
	...
	protected void addPropertyImpl(String key, Object value) {
		...
		if (isDelimiterParsingDisabled()
			...
			if (previousValue != null) {
				addPropertyValues(key, value, isDelimiterParsingDisabled() ? '\0' : getListDelimiter());
			}
		} else {
			addPropertyValues(key, value, isDelimiterParsingDisabled() ? '\0' : getListDelimiter());
	...
	/**
	 * Override the same method in {@link AbstractConfiguration} to simplify the
	 * logic to avoid multiple events being generated. It calls
	 * {@link #clearPropertyDirect(String)} followed by logic to add the property
	 * including calling {@link #addPropertyDirect(String, Object)}.
	 */
	@Override
	...
	/**
	 * Load properties into the configuration. This method iterates through the
	 * entries of the properties and call {@link #setProperty(String, Object)} for
	 * non-null key/value.
	 */
	...
	/**
	 * Copy properties of a configuration into this configuration. This method
	 * simply iterates the keys of the configuration to be copied and call
	 * {@link #setProperty(String, Object)} for non-null key/value.
	 */
	@Override
	 ...
	 * {@link ConfigurationListener#configurationChanged(ConfigurationEvent)} for
	 * all listeners while catching Throwable.
	 */
	@Override
	 ...
	 * {@link ConfigurationErrorListener#configurationError(ConfigurationErrorEvent)}
