...
public interface EurekaClientConfig {
	 ...
	 *
	 * <p>
	 * Eureka servers could be added or removed and this setting controls how soon
	 * the eureka clients should know about it.
	 * </p>
	 *
	...
	/**
	 * Gets the name of the implementation which implements {@link BackupRegistry}
	 * to fetch the registry information as a fall back option for only the first
	 ...
	 * all eureka servers.
	 *
	 * @return total number of allowed connections from eureka client to all eureka
	 *         servers.
	 */
	 ...
	 * eureka server host.
	 *
	 * @return total number of allowed connections from eureka client to a eureka
	 *         server.
	 */
	 ...
	 * contact eureka server when the list of eureka servers come from the DNS.This
	 ...
	 *
	 * <p>
	 * The DNS mechanism is used when {@link #shouldUseDnsForFetchingServiceUrls()}
	 * is set to <em>true</em> and the eureka client expects the DNS to configured a
	 * certain way so that it can fetch changing eureka servers dynamically.
	 * </p>
	 *
	 * <p>
	 ...
	 * </p>
	 *
	 * @return the string indicating the port where the eureka server is listening.
	 */
	 ...
	 * </p>
	 *
	 * @return the string indicating the DNS name to be queried for eureka servers.
	 */
	 ...
	 * </p>
	 *
	 * @return true if the DNS mechanism should be used for fetching urls, false
	 *         otherwise.
	 */
	 ...
	 *
	 * <p>
	 * <em>The changes are effective at runtime at the next registry fetch cycle as
	 * specified by {@link #getRegistryFetchIntervalSeconds()}</em>
	 * </p>
	 *
	 * @return true if the eureka client should prefer the server in the same zone,
	 *         false otherwise.
	 */
	 ...
	 * </p>
	 *
	 * @return true to enable fetching delta information for registry, false to get
	 *         the full registry.
	 */
	 ...
	 * these regions as returned by {@link #getAvailabilityZones(String)}. Failing
	 * to do so, will result in failure of discovery client startup.
	 *
	 * @return Comma separated list of regions for which the eureka registry
	 *         information will be fetched. <code>null</code> if no remote region
	 *         has to be fetched.
	 */
	@Nullable
	...
	/**
	 * Gets the list of fully qualified {@link java.net.URL}s to communicate with
	 * eureka server.
	 *
	 * <p>
	 * Typically the eureka server {@link java.net.URL}s carry
	 * protocol,host,port,context and version information if any.
	 * <code>Example: http://ec2-256-156-243-129.compute-1.amazonaws.com:7001/eureka/v2/</code>
	 * <p>
	 *
	 * <p>
	 * <em>The changes are effective at runtime at the next service url refresh
	 * cycle as specified by {@link #getEurekaServiceUrlPollIntervalSeconds()}</em>
	 * </p>
	 * 
	 ...
	 * {@link com.netflix.appinfo.InstanceInfo.InstanceStatus#UP} states.
	 *
	 * <p>
	 * <em>The changes are effective at runtime at the next registry fetch cycle as
	 * specified by {@link #getRegistryFetchIntervalSeconds()}</em>
	 ...
	 * </p>
	 *
	 * @return time in seconds the connections to eureka can stay idle before it can
	 *         be closed.
	 */
	 ...
	 * eureka server.
	 *
	 * @return {@code true} if registry information has to be fetched, {@code false}
	 *         otherwise.
	 */
	 ...
	 * for a single VIP.
	 *
	 * @return the address of the VIP (name:port). <code>null</code> if single VIP
	 ...
	 * servers
	 *
	 * @return true or false for whether local status updates should be updated to
	 *         remote servers on-demand
	 */
	boolean shouldOnDemandUpdateStatusChange();

	/**
	 * This is a transient config and once the latest codecs are stable, can be
	 * removed (as there will only be one)
	 *
	 * @return the class name of the encoding codec to use for the client. If none
	 *         set a default codec will be used
	 */
	 ...
	 * removed (as there will only be one)
	 *
	 * @return the class name of the decoding codec to use for the client. If none
	 *         set a default codec will be used
	 */
	String getDecoderName();

	/**
	 * @return {@link com.netflix.appinfo.EurekaAccept#name()} for client data
	 *         accept
	 */
	String getClientDataAccept();

	/**
	 * To avoid configuration API pollution when trying new/experimental or features
	 * or for the migration process, the corresponding configuration can be put into
